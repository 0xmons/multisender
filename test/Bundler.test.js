...
contract("ERC1155 deposit/withdraw tests", async accounts => {
  it ("allows depositing tokens and withdrawing, split over several deposits/withdraws", async() => {
    ...
    // Expect balance to now be 100
    await Bundler.withdrawERC1155(Test1155.address, 1, 45, Test721.address, 1, {from: accounts[0]});
    await Bundler.withdrawERC1155(Test1155.address, 1, 45, Test721.address, 1, {from: accounts[0]});
    results = await Test1155.balanceOf(accounts[0], 1);
    expect(results).to.eql(web3.utils.toBN(100));

    // Multiple deposits into 2 and 3
...
// ERC1155 deposit/withdraw (multiple IDs)
...
// Mixed deposit/withdraw (i.e. deposit an ERC20 with the depositERC721 function)
